<template>
  <div class="home">
    <h3 class="search-info" v-if="searchValue!=''">
      Searching for "{{searchValue}}" / {{currentResources.length}} results
...
export default {
  ...
  beforeMount() {
    ...
    axios.get("http://localhost:3000/api/resources").then(response => {
    ...
    filterResources() {
      if (this.searchValue != "" && this.selectedCategories.length != 0) {
        ...
        this.currentResources = this.resources.filter(res => {
          return (
            res.title.includes(this.searchValue) &&
            this.selectedCategories.includes(res.category.id)
          );
        });
